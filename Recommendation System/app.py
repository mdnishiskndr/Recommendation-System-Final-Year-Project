...
app.secret_key = 'Nishii'  # Set a secret key for flash messages
...
def profile(email):
    ...
    if user_profile:
        ...
        name = user_profile[0]
        age = user_profile[2]
        email = user_profile[9]
        athlete = user_profile[4]
        state = user_profile[3]
        sport = user_profile[5]
        rating = user_profile[8]
        time_activity = user_profile[6]

        return render_template('profile.html', name=name, age=age, email=email, athlete=athlete, state=state, sport=sport, rating=rating, time_activity=time_activity)
    else:
...
def Edit_Profile(email):
    ...
    if user_profile:
        ...
        return render_template('Edit_Profile.html', name=name, age=age, email=email, athlete=athlete, state=state, sport=sport, rating=rating, time_activity=time_activity)
    else:
        # Handle the case when the user profile is not found
        return "User profile not found"
...
@app.route('/editprofileform', methods=['POST'])
def editprofileform():
    # Get the user's email from the session (assuming you have stored it in the session)
    email = session.get('email')

    if not email:
        return "User not logged in"  # Handle the case when the user is not logged in

    # Retrieve the sport and rating values from the form data
    ...
    # Update the sport and rating in the database
    query = text("UPDATE userprofile SET Sports = :sport, rating = :rating WHERE email = :email")
    connection.execute(query, {"sport": sport, "rating": rating, "email": email})
...
def recommendation(email):
    ...
    if row:
        ...
        return render_template('NormalRecommendationResult.html', results=top_recs.to_dict('records'))
    else:
        # Handle the case when the user is not found
        return "User not found"

@app.route('/athlete_recommendation/<email>', methods=['GET', 'POST'])
def athlete_recommendation(email):
    # Connect to the database
    engine = create_engine('mysql://root:@localhost:3306/recommendation system')
    connection = engine.connect()
    ...
    if row:
        ...
        # Exclude athletes from the recommended users
        similar_users = similar_users[similar_users["Athlete"] == "Yes"]
        ...
        return render_template('AthleteRecommendationResult.html', results=top_recs.to_dict('records'))
    else:
        # Handle the case when the user is not found
        return "User not found"
